...
image=cv2.imread('imagen_prueba.jpg',0)
#####################################################################
# Max-tree
# ========
...
dictDates = copy.deepcopy(nx_max_tree._node)
padres = copy.deepcopy(nx_max_tree._succ)
hijos = copy.deepcopy(nx_max_tree._pred)

## ADAPTACION PARA EL JOIN
adaptDictToJoin(padres,'padre')
adaptDictToJoin(hijos,'hijos')

panda = pd.DataFrame.from_dict(dictDates)
panda=panda.transpose()
panda=panda.sort_values('value',ascending=False)
panda=panda.reset_index()
#CREAR UN DICTIONARY PARA EL CAMBIO DE INDICES
combi = createCombi(panda)


#CAMBIO DE INDICES DE HIJOS Y PADRES
changeIndexFatherAndSons(hijos,combi,'hijos')
changeIndexFatherAndSons(padres,combi,'padre')
    
panda=pd.DataFrame.set_index(panda,'index')
dictDates=pd.DataFrame.to_dict(panda.transpose())

##JOIN
joinInDict1(dictDates, padres)
joinInDict1(dictDates, hijos)
...
panda=panda.rename(columns={'value': 'Grey Level','label':'pixeles'})
panda=panda.drop(['index'],axis=1)
panda=panda.transpose()
dictDates=pd.DataFrame.to_dict(panda)
maximos,profundidades = dictDeep(dictDates)

areas=changeIndexInKeys(labels_ct,combi)
joinInDict1(dictDates,areas)
pandaDates = pd.DataFrame.from_dict(dictDates)

maximosDeep = dict(zip(maximos, profundidades))

pandaDates=pandaDates.transpose()
pandaDates=pandaDates.drop(['Flag'],axis=1)
